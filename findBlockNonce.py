...
def mine_block(k, prev_hash, transactions):
    ...
    while True:
        ...
        block_data = prev_hash + b''.join(line.encode('utf-8') for line in transactions) + str(nonce_int).encode('utf-8')
        #Get the hash
        block_hash = hashlib.sha256(block_data).digest()
        #Change the hash into binary
        bin_hash = bin(int.from_bytes(block_hash, byteorder='big'))[2:]
        #Check the trailing 0s
